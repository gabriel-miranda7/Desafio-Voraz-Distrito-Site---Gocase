...
const HomePage = () => {
  ...
  return (
    <>
    <NavBar/>
    <div onClick={()=>console.log(products)} className='flex-col items-center'>
      ...
      <h2 className='text-black text-center p-12 text-[24px] font-bold'>You may like</h2>
    </div>
    <div className='flex justify-evenly'>
      {products && products.slice(0,7).map(prod => (
        <ProductCard key={prod.id} name={prod.title} price={prod.price}  image={prod.image}/>
      ))}
    </div>
    </>
  )
